...
interface ProcurementAttributesProps {
  project: Project;
  enterprise: Enterprise;
}

const ProcurementAttributes: React.FC<ProcurementAttributesProps> = ({ project, enterprise }) => {
  ...
  useEffect(() => {
    ...
    const currentAttrs = project.procurementAttributes || [];
  ...
  const handleGlobalSave = async (updatedAttrs: ProjectAttribute[]) => {
    try {
      await projectRepo.update(project.id, { procurementAttributes: updatedAttrs });
    } catch (error) {
  ...
  const handleExport = (attrId: string) => {
    ...
    XLSX.writeFile(wb, `${project.projectCode}_ProcurementAttr_${attr.id}.xlsx`);
  };
      ...
      <div className="flex-1 flex gap-8 p-8 min-h-0">
        ...
        <div className="w-80 bg-white dark:bg-[#141414] border border-gray-200 dark:border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-sm shrink-0">
          <div className="p-4 border-b border-gray-100 dark:border-white/10">
            <h3 className="text-sm font-bold dark:text-white mb-4">Procurement Attributes</h3>
        ...
        <div className="flex-1 bg-white dark:bg-[#141414] border border-gray-200 dark:border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-sm">
          <AnimatePresence mode="wait">
            {selectedAttrId ? (
              ...
              >
                <DataGridModule
                  title={`Procurement Attribute ${selectedAttrId}${selectedAttr?.title ? `: ${selectedAttr.title}` : ''}`}
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-12 text-center text-gray-400">
                 <h3 className="text-lg font-bold">Select an Attribute</h3>
              </div>
            )}
          </AnimatePresence>
        </div>
...
export default ProcurementAttributes;
