...
// We reuse the pattern from ProjectCostCodeAttributes for AttributeTitleInput
...
interface ProgressAttributesProps {
  project: Project;
}

const ProgressAttributes: React.FC<ProgressAttributesProps> = ({ project }) => {
  ...
  useEffect(() => {
    ...
    const currentAttrs = project.progressAttributes || [];
  ...
  const handleGlobalSave = async (updatedAttrs: ProjectAttribute[]) => {
    try {
      await projectRepo.update(project.id, { progressAttributes: updatedAttrs });
    } catch (error) {
      console.error('Error saving progress attributes:', error);
  ...
  const handleExport = (attrId: string) => {
    ...
    XLSX.writeFile(wb, `${project.projectCode}_ProgressAttr_${attr.id}.xlsx`);
  };
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 bg-white dark:bg-[#141414]">
      ...
      <div className="p-6 border-b border-gray-100 dark:border-white/10 flex justify-between items-center bg-gray-50/50 dark:bg-white/5 shrink-0">
        <div>
          <h3 className="text-xl font-bold dark:text-white">Project Commodity Attributes</h3>
          <p className="text-sm text-gray-500">Define the 10 custom metadata attributes for commodity tracking.</p>
        </div>
      </div>
        ...
        <div className="w-80 bg-white dark:bg-[#141414] border border-gray-200 dark:border-white/10 rounded-2xl flex flex-col overflow-hidden shadow-sm">
          <div className="p-4 border-b border-gray-100 dark:border-white/10">
            <div className="relative">
              <Plus className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 rotate-45" />
              <input 
                type="text"
...
export default ProgressAttributes;
