...
const ProjectCostCodeAttributes: React.FC<ProjectCostCodeAttributesProps> = ({ project }) => {
  ...
  useEffect(() => {
    ...
    const initializedAttrs = Array.from({ length: 10 }, (_, i) => {
      ...
      return existing || { id, title: `Attribute ${id}`, values: [] };
    });
                ...
                <div className="p-6 border-b border-gray-100 dark:border-white/10 flex justify-between items-center bg-gray-50/50 dark:bg-white/5 shrink-0">
                  <div>
                    <h3 className="text-xl font-bold dark:text-white">
                      Attribute {selectedAttrId}: {selectedAttr?.title || 'Untitled'}
                    </h3>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead className="bg-black dark:bg-gray-100 sticky top-0 z-20">
                      <tr className="border-b border-white/10 dark:border-black/10">
                        ...
                        <th className="p-3 text-[10px] font-bold uppercase tracking-widest text-white dark:text-black text-right sticky right-0 z-30 bg-black dark:bg-gray-100 border-l border-white/10 dark:border-black/10">Actions</th>
                      </tr>
                    ...
                    <tbody className="divide-y divide-gray-50 dark:divide-white/5">
                      {filteredValues.map((val, index) => (
                        <tr key={val.id} className={cn("hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group", selectedIds.has(val.id) && "bg-blue-50/50 dark:bg-blue-500/5")}>
                          ...
                          {visibleColumns.includes('description') && <td className="p-3 text-xs dark:text-white">{val.description}</td>}
                          {visibleColumns.includes('sortOrder') && <td className="p-3 text-xs dark:text-white">{val.sortOrder?.toString().padStart(2, '0')}</td>}
                          <td className={cn(
