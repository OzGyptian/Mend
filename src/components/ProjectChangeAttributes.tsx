...
interface ProjectChangeAttributesProps {
  project: Project;
}

const ProjectChangeAttributes: React.FC<ProjectChangeAttributesProps> = ({ project }) => {
  ...
  useEffect(() => {
    ...
    const currentAttrs = project.changeAttributes || [];
  ...
  }, [project.changeAttributes]);

  const handleGlobalSave = async (updatedAttrs: ProjectAttribute[]) => {
    try {
      await projectRepo.update(project.id, { changeAttributes: updatedAttrs });
    } catch (error) {
  ...
  const handleExport = (attrId: string) => {
    ...
    XLSX.writeFile(wb, `${project.projectCode}_ChangeAttr_${attr.id}.xlsx`);
  };
...
export default ProjectChangeAttributes;
